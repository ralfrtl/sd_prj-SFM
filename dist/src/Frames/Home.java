...
public final class Home extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        combobox_filter_myfiles.setBackground(new java.awt.Color(255, 255, 255));
        ...
        label_currentfolder_otheruser.setCursor(new java.awt.Cursor(java.awt.Cursor.HAND_CURSOR));
        ...
        label_currentfolder_otheruser.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseReleased(java.awt.event.MouseEvent evt) {
                label_currentfolder_otheruserMouseReleased(evt);
            }
        });
        ...
        table_recent.setForeground(new java.awt.Color(0, 0, 0));
        ...
        input_search.setBackground(new java.awt.Color(255, 255, 255));
    ...
    private void label_currentfolder_otheruserMouseReleased(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_label_currentfolder_otheruserMouseReleased
        try {
//            Socket connection = new Socket(user_ip, Integer.parseInt(user_port));
            ...
            out.writeObject("Load:" + label_currentfolder_otheruser.getName());
            out.flush();
    ...
    }//GEN-LAST:event_label_currentfolder_otheruserMouseReleased
    ...
    private void button_back_otheruserActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_button_back_otheruserActionPerformed
        try {    
            ...
            if(!parent.getParent().equalsIgnoreCase("src")){
...
//                    Socket connection = new Socket(rs.getString("User_IP"), rs.getInt("User_Port"));
                ...
                final JLabel U_IP = new JLabel(""+rs.getInt("User_IP"));
...
//                            OtherUserFiles(null);
        ...
        if (combobox_filter_myfiles.getSelectedIndex()==1){
            ...
            for (int i = path.length - 1; i >= 0; i--){
                if(path[i].isDirectory()){
                    try {
                        ps = con.prepareStatement("SELECT * FROM tbl_trash WHERE File_path = ? AND User_ID = ?");
                        ps.setString(1, path[i].getAbsolutePath());
                        ps.setString(2, label_user.getName());
                        rs = ps.executeQuery();
                        if (rs.next()==false) {
                            FolderList(path[i]);
    ...
    private void FolderListOtherUser(File path){
        final JPanel container = new JPanel();
        container.setBackground(new Color(76,255,137));
        container.setOpaque(false);
        container.setLayout(null);
...
//                                Socket connection = new Socket(user_ip, Integer.parseInt(user_port));
                                Socket connection = new Socket("127.0.0.1", Integer.parseInt(user_port));
                                ObjectOutputStream out = new ObjectOutputStream(connection.getOutputStream());
                                out.writeObject("Load:" + details.getName());
                                out.flush();
                                ...
                                label_currentfolder_otheruser.setText(user_name+"  |  "+details.getText()); //Folder name
                                label_currentfolder_otheruser.setName(details.getName()); //File path
                                
                                OtherUserFiles(file_list); // Use the data colected
                                
                                try {
                                    ps = con.prepareStatement("INSERT INTO tbl_recent (Recent_Details, File_Path, User_ID) VALUES (?, ?, ?)");
                                    ps.setString(1, user_name+" open the folder \'"+details.getText()+"\'.");
                                    ps.setString(2, path.getPath());
            ...
            delete_folder.addActionListener(new java.awt.event.ActionListener() {
                public void actionPerformed(java.awt.event.ActionEvent evt) {
                    ...
                    if (JOptionPane.showConfirmDialog(null, "Are you sure you want to permanently delete the folder \'"+path.getName()+"\' and its contents?\n\nThis action can\'t be undone.","Notice.",JOptionPane.YES_NO_OPTION)==JOptionPane.YES_OPTION){
                        try {
                            File target = new File(path.getAbsolutePath());
